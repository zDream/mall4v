  ...
  methods: {
    ...
    dataFormSubmit () {
      this.$refs['dataForm'].validate((valid) => {
        ...
        // param.integralPrice = 800
        let vmData = this.$http.adornData(param)
        console.log("请求数据",vmData)
        this.$http({
    ...
    paramSetPriceAndStocks (param) {
      ...
      for (let i = 0; i < param.skuList.length; i++) {
        ...
        if (param.integralPrice === 0) {
          param.integralPrice = element.integralPrice ? Number.parseFloat(element.integralPrice) : 0
        }
        // 商品价格为最低价的那件商品的价格
